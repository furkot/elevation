...
function fetchElevationsGet(url, query, api_key) {
  return fetchagent.get(url).query({
...
function fetchElevationsPost(url, query, key) {
  return fetchagent.post(`${url}?api_key=${key}`).send(JSON.stringify(query));
}

export default function init({ url, key, parameters: { get_limit = 150 } = {} }) {
  ...
  async function query(points) {
    ...
    const { height } = await fetchElevations(
      serviceUrl,
      {
        shape: points.map(ll => ({
          lat: ll[1],
